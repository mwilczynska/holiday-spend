...
const METHODOLOGY_SECTIONS: MethodologySection[] = [
  ...
  },
  {
    ...
    bullets: [
      ...
      'Existing stored dataset: 121 cities across 58 countries; v6.1 migrates these rows only through the staged, review-gated pipeline',
    ],
  },
  {
  ...
  },
  {
    ...
    paragraphs: [
      'Each city uses a deterministic panel drawn from an official accommodation register or classification directory and stratified into hostel dorm, hostel private room, and one- through four-star hotel measures. The target is 12 properties per measure and season, with a hard minimum of five accepted public quotes and at least 60% property overlap across seasons. Following the same properties reduces property-mix confounding when seasonal prices change.',
      ...
      'Accommodation prices come from public search-result listings on major booking platforms, currently Booking.com and Trip.com, read signed out and cross-checked between platforms. Prices may also be read from a property\'s own public booking page. Login, member, mobile-only, blocked, or tax-ambiguous rates are recorded as missing rather than worked around.',
      'The first three frozen frames make this design inspectable. Barcelona joins 344 active standard one- through four-star hotels from the July 2026 Catalonia Tourism Register to Barcelona City Council coordinates by official registration id. Of those, 327 geolocate and 322 fall within 5 km of a price-independent median centre. A SHA-256 rank selects 48 primary hotels and preserves 274 ordered reserves; 17 coordinate misses and five radius exclusions remain visible.',
      'Copenhagen uses Hotelstars Union\'s current public Denmark directory. After excluding separately listed conference products and 5-star hotels, 201 national two- through four-star records remain. A median centre from 29 eligible København-labelled hotels fixes the reference point at 55.67250000, 12.56450000; 29 hotels fall within 5 km: three two-star, 11 three-star, and 15 four-star. No one-star property exists in the snapshot, and the full two-star class is below the five-quote minimum, so neither limitation is hidden by an inferred substitute.',
      'VisitCopenhagen supplies a separate 13-property hostel candidate universe. Those links are not yet a ranked hostel panel: each direct site must establish an in-radius address and the relevant dorm and/or private-room inventory. The generic panel contract stores properties separately from measure rankings so one verified hostel can participate in both measures without being counted twice.',
      'Prague exposes and handles duplicate directory records explicitly. The Czech Hotelstars response has 225 eligible one- through four-star rows but only 148 physical properties after grouping normalized name, street, street number, and postcode; 76 identity groups contain duplicates. Four groups with official coordinates disagreeing by more than 0.25 km remain ungeolocated. A median of 18 deduplicated Praha-labelled properties fixes the centre at 50.07870000, 14.43375000.',
      'Prague City Tourism\'s official directory and 12 property pages explicitly support ten in-radius dorm properties and ten private-room properties. Five 3-star and nine 4-star hotels are also eligible; no 1-star or 2-star hotel falls within 5 km. One hostel remains geolocated but inventory-pending because its page states neither room type.',
      'These are sampling milestones, not price or accuracy results. Across Barcelona, Copenhagen, and Prague, no official property website has yet passed verification and no accommodation quote has been accepted. Source-listed links, eligibility, price observation, and validation evidence are deliberately separate states.',
    ],
    bullets: [
      ...
      'Current frozen frames: Barcelona, Copenhagen, and Prague; Copenhagen hostel candidates still await inventory/location checks',
      'Current direct accommodation evidence: 0 verified websites and 0 accepted quotes',
    ],
  },
  {
