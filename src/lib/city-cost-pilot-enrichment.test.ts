...
describe('city cost pilot enrichment', () => {
  ...
  it('contains each of the 36 pilot candidates exactly once', () => {
    ...
    expect(artifact.cities.find((city) => city.city === 'Sendai')?.tourismIntensity).toMatchObject({
      overnightArrivals: 6_508_856,
      residentPopulation: 1_064_142,
      band: 'high',
    });
    expect(artifact.cities.find((city) => city.city === 'Sendai')?.tourismIntensity.value).toBeCloseTo(6.12, 2);
    ...
    expect(artifact.cities.find((city) => city.city === 'Dubai')?.tourismIntensity).toMatchObject({
      status: 'pending_source_collection',
      researchOutcome: 'screened_no_compatible_value',
      rejectionReason: 'incompatible_numerator',
      screenedSources: [
        { name: expect.stringContaining('Annual Visitor Report 2024'), url: expect.stringContaining('dubaidet.gov.ae') },
        { name: expect.stringContaining('Tourism Performance Report'), url: expect.stringContaining('dubaidet.gov.ae') },
      ],
    });
    expect(artifact.cities.find((city) => city.city === 'Nairobi')?.tourismIntensity).toMatchObject({
      status: 'pending_source_collection',
      researchOutcome: 'screened_no_compatible_value',
      rejectionReason: 'incompatible_numerator',
      screenedSources: [
        { name: expect.stringContaining('Tourism Sector Performance Report 2024'), url: expect.stringContaining('tri.go.ke') },
