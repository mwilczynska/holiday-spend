...
describe('city cost methodology v3', () => {
  ...
  it('validates the checked-in official FX snapshot and its cross-rate arithmetic', () => {
    ...
    expect(snapshot.rates.LAK.audPerUnit).toBeCloseTo(
      (1.6312 / 1.1408) / 22515,
      12
    );
    ...
    expect(snapshot.rates.HUF.audPerUnit).toBeCloseTo(1.6312 / 363.9, 12);
    expect(snapshot.rates.MXN.audPerUnit).toBeCloseTo(1.6312 / 19.8767, 12);
    expect(snapshot.rates.TRY.audPerUnit).toBeCloseTo(1.6312 / 53.8658, 12);
    expect(snapshot.rates.INR.audPerUnit).toBeCloseTo(1.6312 / 110.167, 12);
    expect(snapshot.rates.COP.audPerUnit).toBeCloseTo(
      (1.6312 / 1.1408) / 3262.58,
      12
    );
  });
