...
> **PR target**: `main`
> **Status**: IN PROGRESS — plan committed, phases pending
...
- [ ] **Phase 5** — Email delivery (Resend) + deployment docs
...
Extend `getConfiguredAuthProviders()` to return `{ google, emailPassword, devPin }`. `emailPassword` is on whenever the app is past Phase 5 (email delivery configured) OR an env flag `ENABLE_EMAIL_PASSWORD=true` is set. Gate this so phases 2–4 don't accidentally expose sign-in before email delivery is wired.
