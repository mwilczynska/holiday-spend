...
const root = path.resolve("data/reference/v5/experiments/078-expedia-matched-panel");
...
const preference = {"028":1,"029":2,"059":3,"060":4,"061":5,"063":6,"075":7,"078":8};
...
const rows = [];
for (const [experiment, relative] of [...priorRoots, ["078", "data/reference/v5/experiments/078-expedia-matched-panel"]]) {
...
const protocol = inputs.cities.map(({ city }) => {
  ...
  try {
    ...
    return { city, protocolCompliant: payload.schemaVersion === "city-cost-v5-expedia-matched-panel-v1" &&
...
const newRows = pooled.filter((row) => row.experiment === "078");
const result = {
  schemaVersion: "city-cost-v5-expedia-matched-panel-audit-v1",
