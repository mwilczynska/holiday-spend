...
| `node scripts/generate-v6-1-migration-impact-report.mjs [--check]` | `generate-v6-1-migration-impact-report.mjs` | Generates the complete staged 121-city v1-versus-v6.1 operational impact report after Phase 9; refuses incomplete staging and never writes the live CSV or reads a holdout |
| `node scripts/generate-v6-1-migration-impact-report.mjs [--check]` | `generate-v6-1-migration-impact-report.mjs` | Generates the complete staged 121-city v1-versus-v6.1 operational impact report after Phase 9; refuses incomplete staging and never writes the live CSV or reads a holdout |
...
| `node scripts/preregister-v6-1-delegated-canary.mjs --experiment-dir <path> [--collection-mode <mode>]` | `preregister-v6-1-delegated-canary.mjs` | Freezes the delegated canary frame, source window, limits and input/prompt/implementation hashes; refuses a non-empty target directory |
...
| `node scripts/rehearse-city-cost-v6-1-cutover.mjs --check` | `rehearse-city-cost-v6-1-cutover.ts` | Runs a temporary-database insert/replay/rollback rehearsal, checks 121 v6.1 provenance round-trips and proves the live CSV is unchanged; never writes the live DB or CSV |
