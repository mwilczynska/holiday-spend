#!/usr/bin/env node
/** Deterministic audit for Experiment 057; no aggregation or model fitting. */
import fs from 'node:fs';
import path from 'node:path';
const dir = path.join(process.cwd(), 'data', 'reference', 'v5', 'experiments', '057-booking-class-tax-panel');
const classes = ['3', '4'];
...
const promotionGatePassed = classCoverage['3'] >= 8 && classCoverage['4'] >= 8 && completeCities.length >= 6;
console.log(JSON.stringify({ schemaVersion: 'city-cost-v5-booking-class-tax-panel-v1', citiesTested: files.length, classCoverage, completeCities, promotionGate: '3-star>=8, 4-star>=8, complete cities>=6', promotionGatePassed, cities, byClass, productMapping: 'none_source_feasibility_only' }, null, 2));
