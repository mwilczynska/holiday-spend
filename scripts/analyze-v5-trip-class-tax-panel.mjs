#!/usr/bin/env node
/** Deterministic audit for Experiment 058; no aggregation or model fitting. */
import fs from 'node:fs';
import path from 'node:path';

const dir = path.join(process.cwd(), 'data', 'reference', 'v5', 'experiments', '058-trip-class-tax-panel');
const classes = ['2', '3', '4'];
...
console.log(JSON.stringify({ schemaVersion: 'city-cost-v5-trip-class-tax-panel-v1', citiesTested: files.length, classCoverage, completeCities, promotionGate: '2-star>=8, 3-star>=8, 4-star>=8, complete cities>=6', promotionGatePassed, cities, byClass, productMapping: 'none_source_feasibility_only' }, null, 2));
