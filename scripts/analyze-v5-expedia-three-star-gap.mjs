#!/usr/bin/env node
/** Deterministic audit for Experiment 062; no aggregation or model fitting. */
import fs from 'node:fs';
import path from 'node:path';
const dir = path.join(process.cwd(), 'data', 'reference', 'v5', 'experiments', '062-expedia-three-star-gap-panel');
const recoveryCities = new Set(['Cairo', 'Kuala Lumpur', 'Mumbai', 'Santiago']);
...
function valid(row) {
  return Boolean(row?.status === 'found' && Number.isFinite(row.value) && row.value > 0 && /^[A-Z]{3}$/.test(row.currency ?? '') &&
    row.unit === 'per_room_per_night' && row.occupancyBasis === 'explicit_two_adults_source_trend' && row.class === '3_star' &&
...
const promotionGatePassed = accepted.length >= 8 && recoveries.length >= 3;
console.log(JSON.stringify({ schemaVersion: 'city-cost-v5-expedia-three-star-gap-v1', citiesTested: rows.length, acceptedRows: accepted.length, recoveryCities: [...recoveryCities], acceptedRecoveries: recoveries.map((row) => row.city), recoveryCount: recoveries.length, promotionGate: 'overall>=8/12 and recovery>=3/4', promotionGatePassed, rows, productMapping: 'none_coverage_repair_only' }, null, 2));
