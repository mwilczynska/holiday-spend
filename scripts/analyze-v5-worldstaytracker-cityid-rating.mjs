...
const root = path.resolve("data/reference/v5/experiments/083-worldstaytracker-cityid-rating");
...
for (const {city} of inputs.cities) {
  ...
  rows.push({city,complete:accepted.three&&accepted.four,accepted,protocolCompliant:p.schemaVersion==="city-cost-v5-worldstaytracker-cityid-rating-v1"&&t.searchesAttempted===1&&t.searchOperations===1&&t.directReads===2&&t.retries===0&&t.fallbackSources===0&&t.arithmeticOperations===0&&t.currencyConversions===0&&t.crossCityEvidence===0&&t.protocolCompliant!==false});
}
const result={schemaVersion:"city-cost-v5-worldstaytracker-cityid-rating-audit-v1",citiesTested:rows.length,completeCities:rows.filter(r=>r.complete).length,threeStarCoverage:rows.filter(r=>r.accepted.three).length,fourStarCoverage:rows.filter(r=>r.accepted.four).length,protocolCompliant:rows.filter(r=>r.protocolCompliant).length,screenGate:"completeCities>=10 and protocolCompliant>=10",screenGatePassed:rows.filter(r=>r.complete).length>=10&&rows.filter(r=>r.protocolCompliant).length>=10,semanticBasis:"2 adults / 1 night / breakfast included / review score >=7 / 15-day advance",productMapping:"none_semantic_screen_only",rows};
