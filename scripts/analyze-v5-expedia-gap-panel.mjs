...
const root = path.resolve("data/reference/v5/experiments/075-expedia-gap-panel");
...
const preference = {"028":1,"029":2,"059":3,"060":4,"061":5,"063":6,"075":7};
const filesIn = (dir) => !fs.existsSync(dir) ? [] : fs.readdirSync(dir).filter((name) => name.endsWith(".json") && !name.endsWith("-telemetry.json") && !["results.json","audit.json"].includes(name));
...
const strict = (measure, classNumber) => measure?.status === "found" && Number.isFinite(measure.value) && measure.value > 0 &&
  ...
  measure.evidenceText && measure.referencePeriod && measure.searchQuery && ["included","excluded"].includes(measure.taxStatus) &&
  !/\b(?:from|starting|lowest|per person|nearby|regional|weekend|this weekend|event)\b/i.test(measure.evidenceText);
const rows = [];
for (const [experiment, relative] of [...priorRoots, ["075", "data/reference/v5/experiments/075-expedia-gap-panel"]]) {
...
const newRows = pooled.filter((row) => row.experiment === "075");
...
const newCities = inputs.cities.map(({ city }) => {
  const rowsForCity = rows.filter((row) => row.experiment === "075" && row.city === city);
  return { city, strictRows: rowsForCity.length, classes: rowsForCity.map((row) => row.classNumber).sort() };
});
const protocol = inputs.cities.map(({ city }) => {
  const file = path.join(root, `${city.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}.json`);
  try {
    const payload = JSON.parse(fs.readFileSync(file, "utf8"));
    const t = payload.telemetry ?? {};
    return { city, protocolCompliant: payload.schemaVersion === "city-cost-v5-expedia-paired-panel-v1" && t.searchesAttempted === 3 &&
      t.searchOperations === 3 && t.directReads === 0 && t.retries === 0 && t.fallbackSources === 0 && t.arithmeticOperations === 0 &&
      t.currencyConversions === 0 && t.crossCityEvidence === 0 && t.protocolCompliant !== false };
  } catch { return { city, protocolCompliant: false }; }
});
const result = {
  schemaVersion: "city-cost-v5-expedia-gap-panel-audit-v1",
