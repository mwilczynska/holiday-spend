...
if (check) {
  ...
  console.log(JSON.stringify({ passed: true, ...inventory, inventoryFile: path.relative(ROOT, outputPath).replaceAll('\\', '/') }, null, 2));
} else {
  if (write) {
    if (fs.existsSync(outputPath)) throw new Error(`Inventory already exists: ${outputPath}; use --check to validate it.`);
    fs.writeFileSync(outputPath, expected);
  }
  console.log(JSON.stringify({ ...inventory, inventoryFile: write ? path.relative(ROOT, outputPath).replaceAll('\\', '/') : null }, null, 2));
}
