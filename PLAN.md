...
- [ ] Record the live 121-city CSV hash as an invariant.
- [ ] Add a guard proving v1.1 tooling cannot write or regenerate the live CSV.
- [ ] Selectively port live provider/model discovery.
- [ ] Selectively port provider-specific reasoning effort, including `max` where supported.
- [ ] Preserve browser-only API-key handling.
- [ ] Selectively port generic persistence/API provenance helpers.
- [ ] Selectively port deterministic country identity handling.
- [ ] Do not import v5/v6 experiment data, source collectors, canaries, migration tooling, fitting scripts, or release validators.
- [ ] Run the baseline inherited from `main`.
