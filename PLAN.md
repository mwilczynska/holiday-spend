...
Experiment 083 completed with 12/12 protocol-compliant one-city calls, but 0/12 complete cities. The strict audit
accepted six 3-star rows and zero 4-star rows. Every direct `rating=4` read was unsafe or unavailable, so the
pre-registered 10/12 complete-city screen failed. Retain World Stay Tracker 3-star observations and access failures
...
Experiment 084 completed with 12/12 protocol-compliant one-city calls. Nomadlio supplied 64 labelled cells and nine
complete cities, but zero rows were definition-compatible because serving size, party size, tax/fee treatment, and
statistic were not stated. Dubai returned a guide-page misroute; London and Prague each missed one label. Retain all
values as `source_defined_proxy` only; do not substitute coffee/cocktail/wine-bottle labels into frozen estimands. The
next food/drink action must find definition-matched evidence or pre-register a proxy calibration against independent
ground truth.

**Experiment 085 — Expedia exact-heading query contract — complete; rejected.** Twelve independent one-city Luna
contexts issued exactly three Expedia-restricted searches using the indexed heading `Price trends for properties with
N Stars`, plus the source's two-adult and tax wording. All 12 calls were protocol-compliant but produced zero strict
rows: exact pages commonly exposed bare `$` currency, generic/district pages, or wrong-city/class results. The pooled
audit remains 20 matched 2-star←3-star cities and 26 matched 4-star←3-star cities, below both 30-city gates. No
...
**Experiment 086 — Expedia.com bare-dollar currency proxy — complete; promote guarded proxy validation.** Twelve
one-city Luna contexts recorded 27 exact rows (24 bare-dollar proxies, three named USD) without inferring USD in the
prompt. Deterministic exact-host mapping passed the pre-registered same-source screen (median APE 0%, p90 1.72%),
but that comparison is not independent ground truth. Preserve `currency: null` and imputed-currency provenance;
