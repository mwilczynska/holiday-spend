...
- [x] Generate a deterministic 36-city candidate pilot spanning every region and the current cost range
  ...
  - [x] Add a structured, source-attributed tourism-intensity rejection path and record Copenhagen as pending because municipality data expose nights while the city estimate covers international tourists only
...
- Tourism-intensity enrichment now retains structured source-attributed rejections separately from unresearched gaps; Copenhagen is the first recorded rejection because public municipality data expose nights and the published city estimate is international-only
