...
any development or holdout score. `experiments/002-production-prediction-bundle/` records the first exact
production-path run: 0/25 materialized because no local provider credential was configured, with explicit
`not_run` rows.
...
`experiments/005-development-in-sample-score.json` is the scorer output and currently has zero evaluable
tiers because the prediction bundle is empty.
...
street food <- inexpensive               n=6   reasoned k=0.5000 (minimum fitted n=8; grade C ±35%)
...
The current generator requires n>=8 before shipping a fitted relation. The n=6 street-food relation is
retained as a diagnostic but ships the owner-directed reasoned 0.5x inexpensive-meal constant, grade C,
±35%; the n=3 premium relation ships the documented v4 1.5x fallback at grade D, ±45%, rather than its
