# v5 Experiment 057 - Booking class-average tax panel
...
1. `site:booking.com/threestars/city {{CITY}} {{COUNTRY}} 3-star hotels 2 adults 1 room average nightly price taxes fees`
2. `site:booking.com/fourstars/city {{CITY}} {{COUNTRY}} 4-star hotels 2 adults 1 room average nightly price taxes fees`

Do not open pages, retry, use another city, calculate, convert currency, average sources, or use cross-city
evidence. Preserve exact city/class, source URL/title, occupancy wording, price statistic, reference period,
currency, and tax/fee evidence. This is source feasibility only; do not map a product tier or fit a ratio.

Accept a row only when the same evidence explicitly identifies the exact city and requested class, states 2 adults
and 1 room, gives a numeric current class-average nightly room price, and states whether taxes/fees are included or
excluded. Reject generic all-hotel averages, wrong/nearby cities, from/starting prices, event/weekend-only values,
per-person prices, stale/malformed currencies, and unknown tax treatment.

Return JSON only:

```json
{
  "schemaVersion": "city-cost-v5-booking-class-tax-panel-v1",
  "city": "{{CITY}}",
  "country": "{{COUNTRY}}",
...
until a separate validation and aggregation decision passes.
