...
> configured in the process environment. The owner manually generated Wollongong with a browser key, but the local
> feature flag was unset: the persisted row is `source=llm_city_generation`, `methodologyVersion=v1`, with no v6.1
> provenance. This is a failed v6.1 smoke attempt, not v6.1 evidence; the city row is retained. Exact next action:
> restart the local app with the opt-in `CITY_COST_METHODOLOGY_V6=true`, generate 3-5 fresh smoke cities, and verify
> each persisted/API result is v6.1 with full provenance. Never send the key through Codex or store it in the repo.
> Do not execute Phase 11, write the live CSV, access a holdout or change the production/global default until the
> valid smoke and explicit cutover approval are complete.
...
> materialized through the shipped v6.1 path. The batch recorded 96 searches, zero retries and zero direct page reads.
> Direct/fallback counts were accommodation 8/2, food 7/3, drinks 1/9 and activities 7/3. Reykjavik was the only
> all-prior artifact candidate (10%), below the 30% stop rule. Expedia was directly observed in 8/10 cities,
> BudgetYourTrip in 7/10 and Numbeo drinks in 1/10; remaining gaps stayed explicit and used the shipped prior cascade.
> The staged frame is now 110/121; 11 cities remain. The next exact action is fixed batch 010, beginning with Rio de
> Janeiro, Santiago, Havana, New York City, Los Angeles, Montreal, Vancouver, Sydney, Auckland and Queenstown; do not
> recollect completed cities, read a holdout or write the live CSV.
...
> **13 August 2026 Phase 9 batch 008:** Batch 008 is complete and staged. Madrid, Rome, Florence, Athens, Berlin,
> Munich, Amsterdam, Vienna, Paris and London supplied 30 delegated source calls; all ten cities materialized through
> the shipped v6.1 path. The batch recorded 99 searches, zero retries and zero direct page reads. Direct/fallback counts
> were accommodation 8/2, food 7/3, drinks 1/9 and activities 7/3. Berlin and Vienna were all-prior artifact
> candidates (20%), below the 30% stop rule. Expedia was directly observed in 8/10 cities, BudgetYourTrip in 7/10
> and Numbeo drinks in 1/10; remaining gaps stayed explicit and used the shipped prior cascade. The staged frame is
> now 100/121; 21 cities remain. The next exact action is fixed batch 009, beginning with Edinburgh, Copenhagen,
> Stockholm, Helsinki, Reykjavik, Medellin, Bogota, Cartagena, Cusco and Buenos Aires; do not recollect completed
> cities, read a holdout or write the live CSV.
...
> **13 August 2026 Phase 9 batch 006:** Batch 006 is complete and staged. Beijing, Shanghai, Hong Kong, Goa,
> Kathmandu, Amman, Tel Aviv, Marrakech, Nairobi and Zanzibar supplied 30 valid delegated source calls; all ten
> cities materialized through the shipped v6.1 path. The batch recorded 100 searches, zero retries and zero direct
> page reads. Direct/fallback counts were accommodation 5/5, food 8/2, drinks 2/8 and activities 8/2. Hong Kong was
> the only all-prior artifact candidate (10%), below the 30% batch stop rule. Expedia was directly observed in 5/10
> cities, BudgetYourTrip in 8/10 and Numbeo drinks in 2/10; all other gaps stayed explicit and used the shipped prior
> cascade. The staged frame is now 80/121; 41 cities remain. The next exact action is fixed batch 007; do not
> recollect completed cities, read a holdout or write the live CSV.
...
Phase 9 is active at 110/121 staged; batches 007–009 are complete and batch 010 is next. The duplicate Budapest worker report
...
014 passed the fresh canary, so Phase 8 is complete. Phase 9 is active at 110/121 staged and batch 010 is next. The
