...
> **PR target**: `main`
> **Status**: IN PROGRESS -- through Phase 9c
...
5. **argon2id via `argon2` (node-argon2).** Time cost 3, memory 64 MiB, parallelism 1 - tuned later if latency-sensitive.
...
- [x] **Phase 2** - EmailPassword `CredentialsProvider` wired into NextAuth (commit 412f073)
- [x] **Phase 3** - Signup + email verification flow (API + pages) (commit bdbccf4)
...
- [x] **Phase 9a** - Unit tests for password, auth-tokens, and rate-limit (commit 3a998ce)
- [x] **Phase 9c** - CLAUDE.md update (commit 939961b)
...
- `purpose` is a plain string plus a runtime Zod check rather than a CHECK constraint - keeps future purposes (`change_email`, `delete_account`) easy to add.

### 1b. Runtime bootstrap

**File to modify**: `src/db/index.ts`

Add two `CREATE TABLE IF NOT EXISTS` blocks following the existing pattern (see how `saved_plans` and `user_preferences` are done). No backfill needed - these are new data.
...
- `validatePasswordStrength`: min 10 chars, not purely numeric, not in a small denylist of obviously-bad passwords (`password`, `12345678`, etc.). Deliberately minimal - not trying to reinvent a strength meter.
...
CredentialsProvider({
  ...
  async authorize(credentials) {
    ...
    // 3. Look up userPasswords row; if missing - - return null (account exists but has no native password)
    // 4. verifyPassword; if false - - return null
    // 5. If user.emailVerified is null - - throw specific error code 'EMAIL_NOT_VERIFIED'
...
Extend `getConfiguredAuthProviders()` to return `{ google, emailPassword, devPin }`. `emailPassword` is on whenever the app is past Phase 5 (email delivery configured) OR an env flag `ENABLE_EMAIL_PASSWORD=true` is set. Gate this so phases 2- don't accidentally expose sign-in before email delivery is wired.

### 2c. Error surface

NextAuth's `signIn('email-password', ...)` returns `{ error: string }`. Normalize error strings:
- `'EMAIL_NOT_VERIFIED'` - UI links to "resend verification"
- `'CredentialsSignin'` (default NextAuth) - generic "wrong email or password"
...
2. Look up existing user by email
   - If a user exists and has a `userPasswords` row - - return generic `{ ok: true }` (don't leak existence); silently do nothing
   - If a user exists but no `userPasswords` row - - it's an OAuth-only account. Return generic `{ ok: true }` with a flag that the UI can use to suggest "sign in with Google instead"? **No** - keeps enumeration-proof. Just return generic success and send a password-set email? **Decision: return generic success, send no email.** Defer cross-provider linking to Phase 7.
3. If no user exists: insert `user` row (via `ensureUserRow`), insert `userPasswords` row with argon2id hash, issue a `verify_email` token, send verification email (Phase 5 - for now, log the link), return `{ ok: true }`
...
- `consumeToken(token, 'verify_email')` - - if null, return 400 generic error
...
- If user exists + not verified + has a native password - - issue new token, invalidate old `verify_email` tokens for that user, send email

### 3d. Signup page

**File to create**: `src/app/(auth)/signup/page.tsx` (or flat `src/app/signup/page.tsx` if no route group in use - match existing `/login` convention, which is flat)

Form fields: email, password, optional display name. Client-side password strength hint (uses `validatePasswordStrength` client-safe export). Submit - - POST to `/api/auth/signup` - - redirect to `/check-email`.
...
- On success: auto-calls `signIn('email-password', ...)` if possible? **No** - user already authenticated with Google would be confused, and we don't have their password. Instead, show "Email verified. Sign in below." with a link to `/login`.
...
- Handle `EMAIL_NOT_VERIFIED` error - - show "resend verification" CTA
- Keep dev-PIN section unchanged

### Phase 3 verification

- `npx tsc --noEmit` passes
- Browser: `/signup` - - fill form - - email logged to server console with verification link
- Click link - - `/verify-email?token=...` - - "Email verified" - - sign in on `/login` succeeds
- Attempt to sign in before verifying - - `EMAIL_NOT_VERIFIED` error + resend CTA
- Attempt to sign up with existing email - - always returns "check your inbox" (no enumeration)
...
- If user exists and has `userPasswords` row - - issue `reset_password` token (30 min TTL), invalidate prior reset tokens for that user, send email
...
- `consumeToken(token, 'reset_password')` - - if null, 400 generic
...
**Files to create**:
- `src/app/forgot-password/page.tsx` - email input - - POST - - "Check your inbox" screen
- `src/app/reset-password/page.tsx` - reads `?token=...`, new-password field + confirm, POST - - redirect to `/login` with a success flash
...
- (c) Live with the JWT staying valid until its short exp - NextAuth JWT default is 30 days, but we can set `session.maxAge` to something short like 1 hour.
...
- `/forgot-password` with unknown email - - generic success
- `/forgot-password` with known email - - reset email logged (still no real delivery - Phase 5)
- Click reset link - - `/reset-password?token=...` - - set new password - - login with new password works
...
- If `RESEND_API_KEY` is missing in non-production - - log the email to stdout (dev path). In production, throw so misconfiguration is loud.
...
Atomic via SQLite transaction: if window expired - - reset; else increment and compare to `max`.

### 6c. Apply to auth endpoints

Per-endpoint limits (both IP and email buckets - limit on whichever fills first):
...
NextAuth callbacks are tricky to intercept cleanly - the `authorize()` function itself should call `checkRateLimit` as the first thing it does with `'login:email:' + normalizedEmail` and `'login:ip:' + clientIp`.
...
- `npx tsc --noEmit` passes
- Script: hammer `/api/auth/signup` with the same IP - 6th request returns 429 + `Retry-After`
- Script: hammer login with same email, wrong password - limited after 10 attempts
...
On every sign-in:
1. If `account.provider === 'google'` and an existing `user` row owns this email AND that user already has a `userPasswords` row (native account) AND no `account` row links them yet - - **reject** with a specific redirect to `/login?linkRequired=google`.
2. Symmetrical check for email-password sign-in when a Google `account` row exists for the same email but no `userPasswords` row yet - reject with `/login?linkRequired=password`.
3. When the account row already exists for this provider+email pair - - allow (normal flow).
...
- Try Google sign-in with the same email - - redirected to `/login?linkRequired=google`, no session created, no silent linking
...
Sections:
1. **Profile** - email (read-only for now), display name (editable, writes to `user.name`)
2. **Change password** - current password + new password + confirm. Server verifies current hash, rotates it, bumps `tokenVersion`. Only rendered if user has a `userPasswords` row.
3. **Sign-in methods** - read-only list: "Google (linked)" and/or "Email + password (set)". No link/unlink UI yet; "Add [method]" buttons are disabled with "coming soon".
...
- Return `{ ok: true }` - client forces a `signOut()` + redirect to `/login`
...
- Signed-in user - - `/settings/account` - - change password works; old password fails afterwards
...
**Files to create**:
- `src/lib/password.test.ts` - hash/verify round-trip; strength validator edge cases
- `src/lib/auth-tokens.test.ts` - issue/consume/expire/invalidate cycles; idempotent double-consume returns null
- `src/lib/rate-limit.test.ts` - window rollover, per-bucket isolation, concurrent increment
...
Happy-path flow:
1. Signup with new email - - check-email page shown
2. Simulate clicking verification link (hit the verify API directly with a captured token) - - verified
3. Sign in with email+password - - lands on `/`
4. Sign out - - forgot-password flow - - reset token - - new password
5. Sign in with new password succeeds, old password fails

Error paths:
- Signup with existing email - - no enumeration
- Sign in unverified - - `EMAIL_NOT_VERIFIED` + resend works
- Rate limit hit after N attempts - - 429
...
- Push `feat/native-accounts` (push after each phase commit is fine - the plan doc push kicks this off)
