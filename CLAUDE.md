...
- [ ] Complete comparable tourism intensity using same-geography overnight arrivals and resident population
  ...
  - [x] Screen all 18 unmeasured cities and retain explicit rejections where official series do not match the registered numerator
  - [x] Screen or measure the remaining ten unscreened cities without accepting geography or estimand mismatches; zero pilot cities are now unscreened
  - [x] Introduce the graded evidence tier so relaxed-grade measurements stay separable from strict ones in every downstream count
...
- [x] Generate a deterministic 36-city candidate pilot spanning every region and the current cost range
  ...
  - [x] Add Vancouver tourism intensity from Destination Vancouver's exact 2024 monthly overnight-visitation series and BC Stats' same-year municipal population estimate: 11,271,967 / 748,777 = 15.05 (`very_high`), retaining the destination-model methodology limitation
  - [x] Add a structured, source-attributed tourism-intensity rejection path and record Queenstown as pending because official evidence exposes guest nights and daily visitor populations for the wider district, not city arrivals
  - [x] Carry tourism screen outcome, rejection reason, and source URLs into the validated enrichment artifact; report one screened rejection and 23 unscreened gaps separately in the pilot profile
  - [x] Record Auckland as the second screened tourism-intensity rejection because official destination evidence provides international-only arrivals and total guest nights, not all overnight visitor arrivals; profile now reports two screened and 22 unscreened gaps
  - [x] Record Tokyo as the third screened tourism-intensity rejection because official sources expose broad traveler estimates and guest nights, not overnight-only arrivals; profile now reports three screened and 21 unscreened gaps
  - [x] Record Seoul as the fourth screened tourism-intensity rejection because official city evidence provides foreign-only estimated arrivals or broad mobility visits, not all overnight arrivals; profile now reports four screened and 20 unscreened gaps
  - [x] Add Sendai tourism intensity from official 2024 citywide lodging and population reports: 6,508,856 accommodation guests / 1,064,142 residents = 6.12 (`high`), retaining the explicit private-lodging exclusion; profile now reports 13 measured, four screened, and 19 unscreened cities
  - [x] Record Shanghai as the fifth screened tourism-intensity rejection because official statistics provide all domestic tourist trips or international-only overnight arrivals, not all overnight arrivals; profile now reports 13 measured, five screened, and 18 unscreened cities
  - [x] Record Dubai as the sixth screened tourism-intensity rejection because the official visitor series is international-only and the resident-inclusive hotel series publishes room nights rather than all arrivals; profile now reports 13 measured, six screened, and 17 unscreened cities
  - [x] Record Nairobi as the seventh screened tourism-intensity rejection because official evidence provides national arrivals and occupancy or a partial city bed-night series, not all overnight arrivals for Nairobi; profile now reports 13 measured, seven screened, and 16 unscreened cities
  - [x] Record Medellin as the eighth screened tourism-intensity rejection because the official SIT evidence is an eleven-month airport passenger series that includes residents, not full-year all-visitor overnight arrivals for the city; profile now reports 13 measured, eight screened, and 15 unscreened cities
  - [x] Record Delhi as the ninth screened tourism-intensity rejection because official State/UT evidence exposes domestic and foreign tourist visits rather than overnight-only arrivals; profile now reports 13 measured, nine screened, and 14 unscreened cities
  - [x] Add Goa as a whole-state destination-level tourism-intensity proxy from 10,409,197 official 2024 tourist arrivals / 1,583,000 projected residents = 6.58 (`high`), retaining the arrival table's limited inline methodology disclosure; profile now reports 14 measured, nine screened, and 13 unscreened cities
  - [x] Record Havana as the tenth screened tourism-intensity rejection because ONEI's Havana accommodation-arrival series and current national bulletin are international-only, not domestic-plus-international overnight arrivals; profile now reports 14 measured, ten screened, and 12 unscreened cities
  - [x] Record Da Nang as the eleventh screened tourism-intensity rejection because the official accommodation-served series explicitly includes some few-hour rests and is therefore not demonstrably overnight-only; profile now reports 14 measured, 11 screened, and 11 unscreened cities
  - [x] Record Hanoi as the twelfth screened tourism-intensity rejection because its official statistics and tourism-department series expose different incomplete overnight components that cannot be combined without splicing non-reconciled systems; profile now reports 14 measured, 12 screened, and ten unscreened cities
  - [x] Add the graded tourism-intensity evidence tier in enrichment schema v4: every measured record now carries an explicit `evidenceGrade` of `strict` or `relaxed` plus a non-empty `relaxationReasons` list when relaxed, drawn from `geography_approximate`, `numerator_partial`, `numerator_rounded`, and `reference_year_stale`
  - [x] Backfill all 14 pre-existing measured records as `strict` with no relaxation reasons, and enforce the strict/relaxed invariant in both the Zod schema and the enrichment test
  - [x] Clear the final ten unscreened cities in one parallel research pass: four relaxed-grade measurements and six structured rejections, taking unscreened pilot cities from ten to zero
  - [x] Add Can Tho as a relaxed measurement (`numerator_rounded`): over 3,100,000 official 2024 accommodation guests / 1,268,514 residents at the 1 April 2024 mid-term survey = 2.44 (`medium`); geography, period, and visitor scope all match the estimand and only the rounded headline and indirect attribution are relaxed
  - [x] Add Da Lat as a relaxed measurement (`geography_approximate`, `numerator_rounded`): 7,600,000 official 2024 Lam Dong province accommodation guests / 1,361,129 residents = 5.58 (`high`), retaining that the province rather than Da Lat city is measured and that the record must not be reused after the 2025 provincial merger
  - [x] Add Chiang Rai as a relaxed measurement (`geography_approximate`, `numerator_partial`): 6,193,932 reported 2024 provincial visitors / 1,297,657 registered residents = 4.77 (`medium`), retaining that the retrieved report gives a single aggregate without separating overnight tourists from same-day excursionists
  - [x] Add Zanzibar as a relaxed measurement (`numerator_partial`, `reference_year_stale`): 736,755 international 2024 arrivals / 1,889,773 residents at the 2022 census = 0.39 (`low`), retaining that the band reflects the omitted domestic segment and mismatched reference years rather than low tourism dependence
  - [x] Record Bali (Ubud) as a screened rejection because BPS publishes province-level foreign arrivals and domestic tourist trips that include same-day visits, with no Ubud or Gianyar overnight-arrivals series and no retrievable same-year official province denominator
  - [x] Record Vang Vieng and Don Det as screened rejections because Lao official reporting exposes only province visitor totals or forward-looking targets, with no district or village arrivals series and no overnight split
  - [x] Record Pu Luong as a screened rejection because the only site-level figure is a single month of 59,500 guests in October 2024, which cannot be annualized for a strongly seasonal destination
  - [x] Record Santa Fe (Bantayan) as a screened rejection because DOT Region 7 publishes Cebu province arrivals with no municipal or island-level table
  - [x] Record Yangon as a screened rejection because the Ministry of Hotels and Tourism publishes gateway airport arrivals and national foreign totals rather than all-visitor overnight arrivals, the same defect that blocked Medellin
...
- Vancouver raises measured tourism-intensity coverage to 12 of 36 and measured-both-predictors coverage to 11; its exact monthly 2024 destination series and same-year BC Stats municipality estimate produce 15.05 (`very_high`), while the source page's missing inline model methodology remains explicit
- Tourism-intensity enrichment now retains structured source-attributed rejections separately from unresearched gaps; Queenstown is the first recorded rejection because official evidence exposes guest nights and daily visitor populations for the wider district, not city arrivals
- The generated enrichment and model-readiness profile preserve that distinction end-to-end: 12 measured tourism records, one screened rejection, and 23 unscreened pilot gaps
- Auckland is the second structured tourism rejection: official destination evidence provides international-only arrivals and total guest nights; the generated profile now reports 12 measured records, two screened rejections, and 22 unscreened gaps
- Tokyo is the third structured tourism rejection: official sources expose broad traveler estimates and guest nights rather than overnight-only arrivals; the profile now reports 12 measured records, three screened rejections, and 21 unscreened gaps
- Seoul is the fourth structured tourism rejection: official city sources expose foreign-only estimated arrivals and broad tourist-area mobility visits rather than all overnight arrivals; the profile now reports 12 measured records, four screened rejections, and 20 unscreened gaps
- Sendai raises measured tourism-intensity coverage to 13 of 36 and measured-both-predictors coverage to 12; its official 2024 citywide total of 6,508,856 accommodation guests divided by 1,064,142 year-end residents produces 6.12 (`high`), with the source's private-lodging exclusion retained
- Shanghai is the fifth structured tourism rejection: its official 2024 communiqué reports domestic tourist trips without an overnight-only field and international-only overnight arrivals, so the profile now reports 13 measured records, five screened rejections, and 18 unscreened gaps
- Dubai is the sixth structured tourism rejection: DET's official visitor series excludes UAE residents, while its resident-inclusive hotel KPIs expose room nights rather than all arrivals, so the profile now reports 13 measured records, six screened rejections, and 17 unscreened gaps
- Nairobi is the seventh structured tourism rejection: official sources expose national arrivals and occupancy or Nairobi High Class bed-nights rather than all city overnight arrivals, so the profile now reports 13 measured records, seven screened rejections, and 16 unscreened gaps
- Medellin is the eighth structured tourism rejection: its SIT evidence is partial-year airport passenger traffic that includes residents rather than all city overnight arrivals, so the profile now reports 13 measured records, eight screened rejections, and 15 unscreened gaps
- Delhi is the ninth structured tourism rejection: official State/UT evidence exposes domestic and foreign tourist visits rather than overnight-only arrivals, so the profile now reports 13 measured records, nine screened rejections, and 14 unscreened gaps
- Goa raises measured tourism-intensity coverage to 14 of 36 while both-predictor coverage remains 12 because its planner geography is a state rather than a single city; 10,409,197 official 2024 tourist arrivals / 1,583,000 projected residents = 6.58 (`high`), with the arrival table's limited inline methodology disclosure retained
- Havana is the tenth structured tourism rejection: ONEI's provincial accommodation-arrival table and current national bulletin are international-only, so the profile now reports 14 measured records, ten screened rejections, and 12 unscreened gaps
- Da Nang is the eleventh structured tourism rejection: its official 2024 accommodation-served series includes some visitors resting for only a few hours, so the profile now reports 14 measured records, 11 screened rejections, and 11 unscreened gaps
- Hanoi is the twelfth structured tourism rejection: official statistics separate domestic overnight and day guests but not international guests, while the tourism-department series publishes international overnight visitors without a domestic overnight component; the profile now reports 14 measured records, 12 screened rejections, and ten unscreened gaps
- Enrichment schema v4 adds a graded tourism-intensity evidence tier so Phase 6C can trade strictness for throughput without silently contaminating the pilot: `evidenceGrade` is `strict` or `relaxed`, and a relaxed record must name at least one reason from `geography_approximate`, `numerator_partial`, `numerator_rounded`, or `reference_year_stale`
- The strict/relaxed invariant is enforced in three places: a Zod `superRefine` on the measured schema, the enrichment test, and the tourism stratum key in the pilot profile, which now reads `measured_from_public_sources:<grade>:<band>` so relaxed rows never merge into strict strata
- The pilot profile now reports `measuredTourismIntensityStrictCount` and `measuredTourismIntensityRelaxedCount` alongside the total, and its model-readiness reasons state explicitly how many measurements are relaxed-grade and must be excluded from any strict-only fit
- All four new measurements are relaxed-grade: Can Tho 2.44 (`medium`), Da Lat 5.58 (`high`), Chiang Rai 4.77 (`medium`), and Zanzibar 0.39 (`low`); the 14 pre-existing records are backfilled as strict
...
- A three-city protocol checkpoint (Vancouver, Hanoi, Lisbon) validated the venue channel before scaling and produced Amendment E; no observations were written because two protocol gaps blocked a defensible record
- Official venue sites work well for `cocktail_1` and `premium_restaurant_meal_2p`: Pourhouse Vancouver and Moon Men Hanoi publish full tiered cocktail menus, and Botanist Vancouver publishes a CAD 180 tasting menu plus full a la carte
- Amendment E1 adds the within-venue selection rule: take the median of the venue's standard section, excluding premium, reserve, low-ABV, and zero-proof tiers, and record the section name and observed range; without it, selection moves a measure by roughly 2.5x
- Amendment E2 permits static vendor price lists on delivery marketplaces under a new `delivery_platform_menu` source type, ranked below `manual_menu_sample` in every category precedence table, because street vendors structurally do not publish priced sites
- Amendment E3 records that the delivery channel is authorised but currently unretrievable: Uber Eats returns HTTP 403, Fantuan returns an unparseable obfuscated body, and RestaurantGuru has an expired certificate, so `street_food_meal_1p` and its 108 dependent cells remain blocked
- Amendment E4 flags a possible regional bias in the venue channel: official priced menus were readily found in Vancouver and Hanoi but not Lisbon, and that bias would correlate with region, one of the model's stratification variables
- Amendment F supersedes E2/E3 and settles street food with a calibrated ratio model: collect direct evidence wherever a genuine public source exists, then impute the rest from an evidence-estimated `street_food_meal_1p / inexpensive_restaurant_meal_1p` ratio per stratum, which is closer to the original anchor-and-derive approach but with the multiplier estimated rather than asserted
- Seven delivery and aggregator platforms were tested and none exposes prices in server-rendered HTML, so the E2 channel is structurally unexecutable rather than merely awkward
- Three alternatives were rejected on analysis before any collection: redefining street food to a casual-eatery main collapses it into `inexpensive_restaurant_meal_1p` and double-counts in baskets; dropping the term makes `food_street_food` and `food_budget` return identical values; and a fast-food proxy inverts by region, being cheaper than a restaurant meal in Lisbon but dearer than street pho in Hanoi
- `MaterializedCityCostTier` now carries `evidenceBasis` (the weakest `valueStatus` among a cell's inputs) and `imputedMeasures`, so a model-backed tier can never be presented as observed evidence; this guardrail was required before any imputation could run
- Supersession is automatic: `valueStatusPriority` already ranks direct above derived above imputed and the aggregator keeps only the best-priority group, so a later direct observation replaces an imputed one with no manual cleanup
- The ratio model's stability assumption is untested and must be validated by leave-one-city-out error on the anchor set before broad application; if within-stratum dispersion is large, street food reverts to explicitly missing
- The first depth-first collection checkpoint `pilot-wave-02-day-01` adds five accepted observations across Vancouver and Hanoi and lifts pilot coverage from 151 to 156 of 684 cells, newly materializing `food_high_end`, a second and third `drinks_moderate`, `activities_mid_range`, and `activities_high_end`
- Accepted sources are Pourhouse and Botanist and Prince of Whales in Vancouver and Moon Men in Hanoi, all venue-owned or operator-owned pages with published prices; the two cocktail rows apply the Amendment E1 standard-section median with the section range retained in `reportedLow` and `reportedHigh`
- The checkpoint measured the venue channel's yield at roughly one accepted observation per six web calls, which implies the full completion pass at the Amendment B three-venue threshold would need on the order of 4,000 calls, the same order-of-magnitude wall that forced the Amendment C accommodation re-scope
- Lisbon returned zero accepted observations because its cocktail, wine, tasting-menu, and tour prices exist only on Tripadvisor, TheFork, menutoeat, and travel blogs, confirming rather than merely suspecting the Amendment E4 regional venue-channel bias
- Hanoi's official USD 20 three-hour street-food walking tour is retained as an explicit missing outcome rather than an observation, because a second local currency for a VND city is rejected by the aggregator's single-currency invariant
...
- Barcelona's 4-star primary panel now has a source-attributed website outcome for every rank: eleven verified official sites, Marriott's registration-ID resolution for the legacy Husa L'Illa identity, and one validated Sansi Pedralbes placeholder redirect that remains unusable for exact-date quotes; no website outcome is counted as price or quote evidence
